...
## What changed recently in go.mod (Go 1.25–1.27)

- **`go mod init` picks a conservative version** (1.26+): a Go 1.27.x toolchain writes `go 1.26.0`, not `go 1.27.0`, so your module does not accidentally require the newest release. Bump it by hand (`go 1.27`) when you want 1.27-only features — the `stdversion` vet check will remind you.
- **`ignore` directive** (1.25+): tell the `go` command to skip directories it should never treat as packages, such as a frontend build output — the `.npmignore` of modules.
- **Two-block `require` layout** (1.27+, for `go 1.27` modules): `go mod tidy` merges duplicate blocks into exactly one block of direct dependencies and one of `// indirect` ones, keeping attached comments.
