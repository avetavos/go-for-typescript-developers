...
ทุกเมธอดใน `database/sql` มีเวอร์ชันที่รับ Context: `QueryContext`, `QueryRowContext`, `ExecContext` **ใช้เวอร์ชันที่รับ context เสมอ** และส่ง `c.Request().Context()` มาจาก handler ทำให้ database query ถูกยกเลิกได้หากเกิด client disconnect หรือ deadline หมดอายุ — กันงานเสียเปล่าใน connection pool
...
export const quizQuestionsDatabase = [
  ...
  {
    q: 'ทำไมต้องใช้เมธอดเวอร์ชันที่รับ Context ของ database/sql (เช่น QueryContext)?',
    options: [
