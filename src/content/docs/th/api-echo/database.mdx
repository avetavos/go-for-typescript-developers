...
<TsGo ts={tsDbConnect} go={goDbConnect} />
...
ทุก method ใน `database/sql` มีเวอร์ชันที่รับ Context: `QueryContext`, `QueryRowContext`, `ExecContext` **ใช้เวอร์ชันที่รับ context เสมอ** และส่ง `c.Request().Context()` มาจาก handler ทำให้ database query ถูกยกเลิกได้หากเกิด client disconnect หรือ deadline หมดอายุ — กันงานเสียเปล่าใน connection pool
...
export const quizQuestionsDatabase = [
  ...
  {
    q: 'ทำไมต้องใช้ method เวอร์ชันที่รับ Context ของ database/sql (เช่น QueryContext)?',
    options: [
      'เวอร์ชันที่ไม่มี context ถูก deprecated',
      'เพื่อให้ cancellation และ timeout จาก HTTP request ส่งต่อไปได้',
      'เวอร์ชัน context คืนผลลัพธ์แบบ typed',
      'มีแต่เวอร์ชัน context เท่านั้นที่รองรับ parameterized query',
    ],
    answer: 1,
  },
];
