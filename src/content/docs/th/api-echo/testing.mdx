...
func TestGetBook_OK(t *testing.T) {
    ...
    // No router ran, so set the path params by hand (v5: PathValues)
...
func TestGetBook_NotFound(t *testing.T) {
    ...
    // Echo handlers return an error instead of writing to rec —
    // echo.StatusCode reads the status from any HTTP-aware error
...
func TestGetBook(t *testing.T) {
    ...
    for _, tc := range tests {
        t.Run(tc.name, func(t *testing.T) {
            ...
            got := rec.Code
            if err := h.Get(c); err != nil {
                got = echo.StatusCode(err) // error path: status lives on the error
            }
